...
export default class SimEngine {
  ...
  _tickSignals(now) {
    ...
    const nsLoad =
      this.vehs.filter(v => v.dir === 0).length +
      this.vehs.filter(v => v.dir === 2).length;

    const ewLoad =
      this.vehs.filter(v => v.dir === 1).length +
      this.vehs.filter(v => v.dir === 3).length;
  ...
  _onPhaseSwitch() {
    ...
    const rn = ns ? 'A & C' : 'B & D';
    // FIX: use real vehicle counts from this.vehs instead of fake this.dens
    const load = ns
  ...
  _spawnV(dir) {
    ...
    // SPAWN FIX: use axis-appropriate far distances.
    // Using max(CW,CH) for NS roads on a wide canvas puts spawn y way outside
    // the ±150 despawn boundary → vehicles killed the same frame they are created.
    const farNS = this.CH * 0.52;   // height-based: safe for vertical roads
    const farEW = this.CW * 0.52;   // width-based:  safe for horizontal roads

    let px, py, vx, vy;
    if (dir === 0) { px = this.cx() + lane; py = this.cy() - farNS; vx = 0; vy = VEH_SPD; }
    else if (dir === 2) { px = this.cx() + lane; py = this.cy() + farNS; vx = 0; vy = -VEH_SPD; }
    else if (dir === 1) { px = this.cx() + farEW; py = this.cy() + lane; vx = -VEH_SPD; vy = 0; }
    else { px = this.cx() - farEW; py = this.cy() + lane; vx = VEH_SPD; vy = 0; }
  ...
  _maybeSpawn(dt) {
    for (let d = 0; d < 4; d++) {
      ...
      if (this.spawnTick[d] <= 0) {
        const vehiclesInDir = this.vehs.filter(v => v.dir === d).length;

        if (vehiclesInDir < MAX_PER_DIR) {
          this._spawnV(d);
          this.spawnTick[d] = 1200 + Math.random() * 800;
        } else {
  ...
  _updateEmgVehs(dt, now) {
    ...
    if (this.overrideActive && now >= this.overrideEnd) {
      ...
      this.sigState = 'NS_GREEN';
      // FIX: use real vehicle counts instead of fake this.dens
      const nsReal = this.vehs.filter(v => v.dir === 0 || v.dir === 2).length;
      this.sigDur = Math.min(14000, 5000 + nsReal * 220);
      this.phaseStartTime = now;
    }
    if (this.manualOverrideDir >= 0 && now >= this.manualOverrideEnd) {
      this.manualOverrideDir = -1;
  ...
  spawnEmergency(type, dir) {
    ...
    const lane = this.laneOffset(dir);
    // SPAWN FIX: same axis-appropriate far as _spawnV
    const farNS = this.CH * 0.52;
    const farEW = this.CW * 0.52;
    let px, py;

    if (dir === 0) { px = this.cx() + lane; py = this.cy() - farNS; }
    else if (dir === 2) { px = this.cx() + lane; py = this.cy() + farNS; }
    else if (dir === 1) { px = this.cx() + farEW; py = this.cy() + lane; }
    else { px = this.cx() - farEW; py = this.cy() + lane; }

    const v = {
      id: uid(), type, dir, px, py, speed: cfg.spd, passed: false,
